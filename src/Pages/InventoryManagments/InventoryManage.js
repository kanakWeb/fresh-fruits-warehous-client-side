...
const InventoryManage = () => {
  ...
  useEffect(() => {
    const url = `https://whispering-scrubland-68201.herokuapp.com/inventoryItem/${itemId}`;
    // console.log(url);
    fetch(url)
      .then((res) => res.json())
  ...
  const handleFormSubmit = (event) => {
    ...
    if (!quantity) {
      ...
      const newQuantity =
        parseInt(items.quantity) + parseInt(quantity);
      const newQuantityObj = { newQuantity };

      fetch(
        `https://whispering-scrubland-68201.herokuapp.com/inventoryItem/${itemId}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelivery = (itemId) => {
    ...
    if (quantity > 0) {
      ...
      const url = `https://whispering-scrubland-68201.herokuapp.com/delivery/${itemId}`;
      fetch(url, {
        method: "PUT",
        headers: {
          "content-type": "application/json",
